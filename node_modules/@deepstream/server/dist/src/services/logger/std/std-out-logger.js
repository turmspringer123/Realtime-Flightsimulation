...
class StdOutLogger extends types_1.DeepstreamPlugin {
     ...
     */
    constructor(options = {}, services, config) {
        super();
    ...
    log(logLevel, namespace, event, logMessage, metaData = null) {
        if (logLevel >= types_1.LOG_LEVEL.WARN && this.services) {
        ...
        if (this.useColors) {
            process[outputStream].write(chalk[this.logLevelColors[logLevel]](msg) + os_1.EOL);
        }
        else {
            process[outputStream].write(msg + os_1.EOL);
        }
        if (logLevel === types_1.LOG_LEVEL.FATAL) {
            this.services.notifyFatalException();
        }
    }
}
